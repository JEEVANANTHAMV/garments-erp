...
export default function YarnGRNDetailPage() {
  ...
  const totals = useMemo(() => {
    ...
    const grandTotal = Math.round((taxableAmount + totalTax + (Number(header.freight_charges) || 0) + (Number(header.other_charges) || 0) + (Number(header.round_off) || 0)) * 100) / 100;
  ...
  }, [lines, header.is_interstate, header.freight_charges, header.other_charges, header.round_off]);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...header,
        freight_charges: header.freight_charges,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
            <div>
              ...
              <input
                ...
                className="w-full text-xs font-mono rounded-lg border border-slate-300 py-1.5 px-2 text-right bg-white"
                placeholder="0.00"
              />
            </div>
