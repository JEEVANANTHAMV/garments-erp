...
async function seedDemo(ctx: {
  ...
  const YARN_COUNTS: [string, 'Ne' | 'Nm' | 'Denier' | 'Tex', string, number][] = [
    ...
    ['2/30s', 'Ne', 'Double / 2-Ply Cotton Count', 31],
    ['2/40s', 'Ne', 'Double / 2-Ply Combed Count', 41],
    ['2/50s', 'Ne', 'Double / 2-Ply Superfine Count', 51],
    ['2/60s', 'Ne', 'Double / 2-Ply Fine Count', 61],
    ['30s/30s/10s', 'Ne', '3-End Fleece / French Terry Composite Count', 32],
    ['34s/34s/10s', 'Ne', 'French Terry Loop Composite Count', 35],
    ['40s/40s/20s', 'Ne', 'Fleece Loop Composite Count', 42],
    ['20D', 'Denier', 'Spandex / Lycra Filament', 20],
    ['30D', 'Denier', 'Spandex / Lycra Filament', 30],
    ['40D', 'Denier', 'Spandex / Lycra Filament', 40],
  ...
  for (const [soNo, buyer, soDate, shipDate, season, curCode, useAgent, lines] of ORDERS) {
    ...
    const r = await exec(
      `INSERT INTO trx_sales_order (
         company_id, branch_id, so_no, io_no, order_type,
         so_date, buyer_id, agent_id, buyer_po_no, buyer_po_date,
         season, currency_id, exchange_rate, incoterm, port_of_loading,
         destination_country, destination_port, payment_term, ship_date, delivery_date,
         status_id, approval_state, created_by
       ) VALUES (
         ?, ?, ?, ?, 'EXPORT',
         ?, ?, ?, ?, ?,
         ?, ?, ?, 'FOB', 'Tuticorin',
      ...
      [
        companyId, hoBranch, soNo, ioNo,
        soDate, partyId.get(buyer), useAgent ? partyId.get('A001') : null, `PO-${buyer}-${soNo.slice(-4)}`, soDate,
        season, curId, curCode === 'USD' ? 83.2 : curCode === 'EUR' ? 90.1 : 105.4,
        buyerRow?.country_id ?? null, shipDate, shipDate,
        soApproved, adminId,
      ]
    );
