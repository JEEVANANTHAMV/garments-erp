...
const LOOKUPS: Record<string, LookupDef> = {
  ...
  'sizes-all':   { sql: `SELECT sz.id, sz.size_code AS code, CONCAT(sz.size_label,' (',g.group_name,')') AS label, sz.sort_order, sz.size_code, sz.size_label, g.group_name FROM mst_size sz JOIN mst_size_group g ON g.id=sz.size_group_id WHERE g.company_id=? AND sz.is_active=1 ORDER BY g.group_name, sz.sort_order, sz.id`, scoped: true },
