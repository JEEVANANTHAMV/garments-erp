...
export const transactionResources: ResourceConfig[] = [
  ...
  },
  {
    ...
    joins: `LEFT JOIN mst_party b   ON b.id   = t.buyer_id
            ...
            LEFT JOIN cfg_status cs ON cs.id  = t.status_id`,
    children: [
      { key: 'lines', table: 'trx_quotation_line', fk: 'quotation_id',
        ...
        fields: [
          ...
          f('gsm', s.nullableStr(30)),
          f('yarn_type', s.nullableStr(80)),
          f('yarn_count', s.nullableStr(50)),
          f('trim_size', s.nullableStr(50)),
          f('description', s.nullableStr(255)),
          f('qty', s.decReq()), f('uom_id', s.id()),
          f('unit_price', s.decReq()),
          f('quotation_rate', s.dec()), f('confirm_rate', s.dec()),
          f('gst_rate', s.dec()), f('gst_amount', s.dec()),
    ...
    fields: [
      ...
      f('agent_id', s.id()), f('enquiry_id', s.id()),
      f('currency_id', s.idReq()), f('exchange_rate', s.dec()),
      f('incoterm', s.enum(INCOTERM)),
      f('port_of_loading', s.nullableStr(100)), f('port_of_discharge', s.nullableStr(100)),
      f('valid_until', s.date()), f('payment_terms', s.nullableStr(150)),
      f('job_no', s.nullableStr(60)),
      // Domestic summary
      f('discount_pct', s.dec()), f('discount_amount', s.dec()),
      f('freight_charges', s.dec()), f('packing_charges', s.dec()), f('other_charges', s.dec()),
      f('taxable_value', s.dec()),
      f('cgst_rate', s.dec()), f('cgst_amount', s.dec()),
      f('sgst_rate', s.dec()), f('sgst_amount', s.dec()),
      f('igst_rate', s.dec()), f('igst_amount', s.dec()),
      f('round_off', s.dec()),
      // Import summary
      f('courier_charges', s.dec()), f('insurance', s.dec()),
      f('bank_charges', s.dec()), f('customs_duty', s.dec()), f('clearing_charges', s.dec()),
      f('landed_cost', s.dec()), f('margin_pct', s.dec()), f('final_selling_rate', s.dec()),
  ...
  // ------------------------------------------------ Procurement
  {
    ...
    fields: [
      ...
      f('is_interstate', s.bool()),
      f('freight_charges', s.dec()), f('other_charges', s.dec()), f('round_off', s.dec()),
      f('tcs_applicable', s.bool()), f('tcs_section', s.nullableStr(30)),
      f('tcs_rate', s.dec()), f('tcs_amount', s.dec()),
  ...
  },
  {
    ...
    fields: [
      ...
      f('gate_inward_id', s.id()), f('is_interstate', s.bool()),
      f('subtotal', s.dec()), f('discount_amount', s.dec()), f('tax_amount', s.dec()),
  ...
  // ------------------------------------------------ Supplier Bill (Bills Inward)
  {
    ...
    children: [
      { key: 'lines', table: 'trx_supplier_bill_line', fk: 'bill_id', fields: [
        ...
        f('no_of_rolls', s.dec()), f('dia', s.nullableStr(30)),
        f('gsm', s.dec()), f('color_name', s.nullableStr(100)),
        f('size_name', s.nullableStr(50)),
    ...
    fields: [
      ...
      f('gst_type', s.enum(['INTRA_STATE','INTER_STATE','IMPORT'])),
      f('exchange_rate', s.dec()),
      f('subtotal', s.dec()), f('gst_amount', s.dec()),
      f('tds_amount', s.dec()), f('total_amount', s.dec()),
      f('base_currency_total', s.dec()),
      f('boe_no', s.nullableStr(60)), f('boe_date', s.date()), f('port_code', s.nullableStr(30)),
